...
CellCountX の配布ファイルは、GitHub の 2GB 制限を回避するため**複数の ZIP ファイル（.zip.001, .zip.002, ...）に分割されています。**

インストールするには、以下の手順で **分割 ZIP を結合 → 展開**してください。

### 📝 1. ダウンロードするファイル

Release ページから、以下のような複数ファイルをすべてダウンロードしてください：

```bash
CellCountX-v*.*.*.zip.001
CellCountX-v*.*.*.zip.002
...
```
...
→ **7-Zip**
→ **「展開」または「ここに展開」**
3. 自動的に `.zip.002` 以降も読み込まれ、
**CellCountX フォルダが展開されます。**

#### 方法 B：コマンドラインで結合（上級者向け）

⚠ PowerShell では動きません

PowerShell で `copy /b` を実行するとエラーになります。

✔ cmd.exe（コマンドプロンプト）で実行してください
...
3. 以下を実行：
    ...
    ```bash
    copy /b CellCountX-v0.9.1-beta.zip.001 + CellCountX-v0.9.1-beta.zip.002 CellCountX.zip
    ```
...
- `.zip.001` だけでは展開できません。
**すべての分割ファイルが必要です。**
- 分割ファイルは **同じフォルダに置く必要があります。**
- 展開には **7-Zip の使用を推奨**します（Windows 標準の解凍では失敗する場合があります）。
