...
高精度な細胞カウントを GUI で実行できる WPF アプリケーションです。バックエンドには **Python（Embeddable 版）+ CellPose + PyTorch** を使用し、画像フォルダを指定するだけで **バッチ処理・GPU 対応・CSV 出力** を自動で行います。

Python はアプリに完全同梱されているため、**ユーザー側で Python をインストールする必要はありません。**
...
- CellPose 推論
- 非接着細胞除去フィルタ（nonadherent_filter.py）
- マスク画像保存
- **輪郭オーバーレイ画像保存**
- JSON で結果を返す
...
CellCountX は配布版で Embeddable Python を使用します。開発時には、リポジトリに含まれる以下の ZIP を展開してください。
...
5. `server.py` と `nonadherent_filter.py` を exe と同じ場所にコピー
...
GitHub の 2GB 制限を回避するため、CellCountX は **複数の ZIP（.zip.001, .zip.002, ...）に分割**されています。
...
#### コマンドラインで結合する方法（上級者向け）

cmd.exe（コマンドプロンプト）で実行してください。PowerShell で `copy /b` を実行するとエラーになります。

1. Windows の検索で **cmd** と入力して「コマンドプロンプト」を開く
2. 分割 zip があるフォルダへ移動
3. 以下を実行：
...
## 🧬 非接着細胞除外フィルタ（nonadherent_filter.py）

CellCountX では、CellPose のマスクに対して以下の 4 指標を計算し、いずれかの条件を満たした細胞を非接着細胞として除外します。

| 指標 | 説明 | 除外条件 |
| --- | --- | --- |
| **min_area** | 細胞領域の面積（px） | ``area ``< ``min_area`` |
| **max_circularity** | 円形度（4πA/P²）。非接着細胞は丸くなる傾向 | ``circularity ``> ``max_circularity`` |
| **max_intensity** | 平均輝度。非接着細胞は明るくなる傾向 | ``mean_intensity ``> ``max_intensity`` |
| **min_variance** | 輝度分散。非接着細胞は内部が均一化 | ``variance ``< ``min_variance`` |

これらは OR 条件で適用され、1 つでも閾値を満たさない場合は除外されます。
